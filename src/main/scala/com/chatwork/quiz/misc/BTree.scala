...
case class Branch(left: Node, value: Int, right: Node) extends Node {
  ...
  val min: Int = List(left.min, right.min, value).min

  def find(value: Int): Option[Node] = if (this.value == value) Some(this) else {
...
case class Leaf(value: Int) extends Node {
  ...
  def find(v: Int): Option[Node] = if (value == this.value) Some(this) else None
